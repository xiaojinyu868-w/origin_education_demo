...
const RosterSetup = () => {
    ...
    const examPreview = useMemo(() => {
        return questionDrafts.map((item, index) => (_jsxs(List.Item, { children: [_jsx(List.Item.Meta, { title: `第 ${item.number} 题 · ${renderQuestionType(item.type)}`, description: _jsxs(Space, { direction: "vertical", style: { width: "100%" }, children: [_jsx(Paragraph, { style: { marginBottom: 0 }, children: item.prompt || "题干待完善" }), _jsxs(Text, { type: "secondary", children: ["\u77E5\u8BC6\u70B9\uFF1A", item.knowledge_tags || "未标注"] }), _jsxs(Text, { type: "secondary", children: ["\u6EE1\u5206 ", item.max_score, " \u5206"] }), _jsxs(Text, { type: "secondary", children: ["\u9002\u7528\u8303\u56F4\uFF1A", (item.target_student_ids && item.target_student_ids.length > 0) ? item.target_student_ids
                                        .map((id) => studentLookup.get(id)?.name || `学生 #${id}`)
                                        .join("、") : "默认全班"] })] }) }), _jsx(Button, { type: "link", danger: true, onClick: () => handleRemoveQuestion(index), children: "\u5220\u9664" })] }, `${item.number}-${index}`)));
    }, [questionDrafts]);
