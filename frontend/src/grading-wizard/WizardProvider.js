...
const extractWizardProgressPayload = (payload) => {
    ...
    const parseSegment = (node) => {
        ...
        const updatedAt = typeof node.updatedAt === "string"
            ? node.updatedAt
    ...
    return {
        ...
        review: parseSegment(asRecord(progressRoot.review) ??
            asRecord(progressRoot.reviewSummary) ??
            asRecord(progressRoot.aiReview)),
    };
};
const mergeWizardProgressSegments = (existing, incoming) => {
...
const mergeSessionPayload = (base, update) => {
    const baseRecord = asRecord(base) ?? {};
    const updateRecord = asRecord(update) ?? {};
    const merged = { ...baseRecord, ...updateRecord };
    if ("wizardProgress" in baseRecord || "wizardProgress" in updateRecord) {
        merged.wizardProgress = mergeWizardProgressSegments(baseRecord["wizardProgress"], updateRecord["wizardProgress"]);
    }
    return merged;
};
const buildProgress = (state) => {
    ...
    const answersConfirmed = selectedExam?.questions?.filter((question) => question.answer_status === "confirmed").length ?? 0;
    ...
    const uploadsCompletedRaw = Math.max(0, payload.uploads?.completed ??
    ...
    const uploadsAny = Math.max(0, uploadsTotalRaw ?? 0, uploadsCompletedRaw + (uploadsPendingRaw ?? 0));
    const normalizedUploadsCompleted = Math.max(0, Math.min(uploadsAny, uploadsCompletedRaw));
    const normalizedUploadsPending = Math.max(0, uploadsPendingRaw ?? Math.max(uploadsAny - normalizedUploadsCompleted, 0));
    ...
    const reviewTotal = Math.max(0, payload.review?.total ??
        (typeof payload.review?.confirmed === "number" || typeof payload.review?.pending === "number"
            ...
            : 0));
    const reviewPending = Math.max(0, payload.review?.pending ?? Math.max(reviewTotal - (payload.review?.confirmed ?? 0), 0));
    const reviewConfirmed = Math.max(0, Math.min(reviewTotal, (payload.review?.confirmed ?? Math.max(reviewTotal - reviewPending, 0))));
...
const computeBlockingForStep = (step, context) => {
    const reasons = [];
...
const computeBlockingMap = (state, progress) => {
    const selectedExam = state.exams.find((exam) => exam.id === state.selectedExamId);
    const context = {
...
const resolveAccessibleStep = (desired, blocking) => {
...
export const WizardProvider = ({ children }) => {
    const [state, setState] = useState(() => applyDerivedState(baseInitialState));
    ...
    const initialize = useCallback(async (options) => {
        ...
        try {
            ...
            if (!preferredTeacherId) {
                setState((prev) => applyDerivedState({
                    ...prev,
            ...
            const normalizedExamId = typeof session.exam_id === "number" ? session.exam_id : snapshot.selectedExamId;
    ...
    const goToStep = useCallback(async (step, options = {}) => {
        ...
        try {
            ...
            let nextState = applyDerivedState({
                ...
                session,
                step: clampStep(session.current_step),
                selectedExamId: typeof session.exam_id === "number" ? session.exam_id : nextSelectedExamId,
                savingStep: false,
                error: undefined,
            });
