...
本需求旨在为教师批改闭环提供一条从首页入口到历史回放的沉浸式向导流程，并确保对外可见的用户引导与对内实现细节均被清晰记录，为后续产品、设计与研发协作奠定基础。
...
1. When 教师点击“开始新一轮批改”CTA时, the 批改向导壳 shall 全屏展示并隐藏侧边栏，仅保留品牌与退出入口。
2. When 批改向导显示时, the 界面 shall 展示引导文案与当前流程进度提示，确保教师清楚所处阶段。
...
1. When 教师进入 Step 1 时, the 页面 shall 展示“选择已有试卷”卡片列表与“新建试卷”按钮，并默认聚焦已有试卷列表。
2. When 教师选中某份试卷时, the 界面 shall 在右侧显示试卷摘要（名称、题数、上次更新时间），并在确认前阻止进入下一步。
3. While 教师选择“新建试卷”, when 试卷图片上传完成时, the 系统 shall 生成 JSON 解析结果填充标题、科目与题目列表，并允许人工编辑和确认后返回主流程。
...
1. When 进入 Step 2 时, the 页面 shall 展示单题大卡，包括题号、系统答案（按题型渲染）与人工编辑区域，同时显示剩余题目进度。
...
2. When 系统接收到新扫描件时, the 平台 shall 自动匹配试卷模板与学生信息并显示置信度提示，若置信度低则要求人工确认。
...
2. While 教师确认题目, when 点击“确认批改”按钮时, the 系统 shall 保存该题状态并自动定位到下一题。
3. When 存在置信度低或待确认题目时, the 题目列表 shall 默认置顶显示相关条目并提供快速过滤。
...
1. When 所有题目确认完成时, the 系统 shall 展示成功页面并提供“导出批改报告”“派送错题练习”“回到批改历史”三个 CTA。
2. When 成功页面展示时, the 文案 shall 告知本次批改已存入历史记录并可回看。
...
2. When 教师点击“查看详情”时, the 系统 shall 以只读模式打开结果工作台，呈现当时 AI 与人工记录。
...
3. When 展示全局流程时, the 界面 shall 始终提供 Breadcrumb“试卷选择 → 答案校验 → 上传试卷 → 批改确认 → 完成导出”，并以状态标记已完成步骤。
